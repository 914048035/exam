		...
		<div class="nr_left">
			<div class="test">
				<form action="" method="post">
					...
					<c:if test="${exam.lunshuQuestionNum !=0 }">
						<div class="test_content">
							<div class="test_content_title">
								<h2>论述题</h2>
								<p>
									<span>共</span>
									<i lass="content_lit">${exam.lunshuQuestionNum }</i>
									<span>题，</span><span>合计</span><i
										class="content_fs">${lunshuScore * exam.lunshuQuestionNum}</i><span>分</span>
								</p>
							</div>
						</div>
						<div class="test_content_nr">
							<ul>
								<c:forEach items="${lunshuQuestionList}" var="cq" varStatus="cqids">
									<li id="qu_${lunshuQuestion }_${cq.question.id}" data-key="${cq.id}">
										<div class="test_content_nr_tt">
											<i>${cqids.index+ exam.singleQuestionNum+ exam.muiltQuestionNum+ exam.chargeQuestionNum +1}</i>
											<span>(${cq.question.score}分)</span><font>${cq.question.title }</font>
										</div>

										<div class="test_content_nr_main">
											<input type="text" class="text-input" name="answer${cq.id}"
												   id="${lunshuQuestion }_answer_${cq.id}_option_1"
												   readonly value="${cq.answer}"
					...
					<c:if test="${exam.lunshuQuestionNum !=0 }">
						<div class="rt_content">
							<div class="rt_content_tt">
								<h2>论述题</h2>
								<p>
									<span>共</span><i class="content_lit">${exam.lunshuQuestionNum }</i><span>题</span>
								</p>
							</div>
							<div class="rt_content_nr answerSheet">
								<ul>
									<c:forEach items="${lunshuQuestionList}" var="ccq" varStatus="ccqids">
										<li>
											<a href="#qu_${lunshuQuestion}_${ccq.question.id}">
													${ccqids.index + exam.singleQuestionNum + exam.muiltQuestionNum + exam.chargeQuestionNum + 1}</a>
										</li>
									</c:forEach>
								</ul>
							</div>
...
<p>【嘻哈】在线考试系统</p>
<p><a href="/home/user/logout">【嘻哈】</a></p>
<%--    <p><%   response.setHeader("Refresh", "1;url=page2.jsp"); %>【嘻哈】</p>--%>
</div>
</body>
</html>
