...
import com.java.programmer.service.admin.SubjectService;
...
public class ExamController {
	...
	public Map<String, String> add(Exam exam){
		...
		if(exam.getSingleQuestionNum() == 0 && exam.getMuiltQuestionNum()
				== 0 && exam.getChargeQuestionNum() == 0 && exam.getLunshuQuestionNum() == 0){
			ret.put("type", "error");
			ret.put("msg", "单选题、多选题、判断题,论述题至少有一种类型的题目!");
			return ret;
		}
		...
		//获取论述题总数
		queryMap.put("questionType", Long.valueOf(Question.QUESTION_TYPE_LUNSHU));
		int lunshuQuestionTotalNum = questionService.getQuestionNumByType(queryMap);
		if(exam.getLunshuQuestionNum() > lunshuQuestionTotalNum){
			ret.put("type", "error");
			ret.put("msg", "论述题数量超过题库判断题总数，请修改!");
			return ret;
		}
		exam.setQuestionNum(exam.getSingleQuestionNum() + exam.getMuiltQuestionNum() +
				exam.getChargeQuestionNum() + exam.getLunshuQuestionNum());
		exam.setTotalScore(exam.getSingleQuestionNum() * Question.QUESTION_TYPE_SINGLE_SCORE
				+ exam.getMuiltQuestionNum() * Question.QUESTION_TYPE_MUILT_SCORE
				+ exam.getChargeQuestionNum() * Question.QUESTION_TYPE_CHARGE_SCORE
				+ exam.getLunshuQuestionNum() * Question.QUESTION_TYPE_LUNSHU_SCORE
		);
